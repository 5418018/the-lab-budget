...
target_file = sorted(excel_files, reverse=True)[0]
print(f"변환 대상 파일: {target_file}")
...
for r in range(6, 14):
    ...
    if month:
        member_stats.append({
            ...
            'totalIncome': float(tot_inc) if tot_inc is not None else 0,
            'note': str(note).strip() if note else ''
        })

last_balance = transactions[-1]['balance'] if transactions else 31471771
official_spent = sum(item['spent'] for item in dashboard_items)
total_budget = sum(item['budget'] for item in dashboard_items)

dataset = {
    'updatedAt': datetime.now().strftime('%Y-%m-%d'),
    'title': 'THE 연구소 5기 회계 및 실시간 예산 현황',
    'kpi': {
        'totalBudget': 41218091,
        'officialSpent': 6832580,
        'provisionalSpent': 2913740,
        'totalSpent': 9746320,
